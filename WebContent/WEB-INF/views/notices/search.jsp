...
<div class="pageHeader">
	<form class="advancedSearchForm" style="display: none;" rel="pagerForm_hidden"  onsubmit="return navTabSearch(this);" action="${webUrl}/notices" method="POST">
	<div class="searchBar">
<%-- ===================修改搜索条件===================== --%>
...
<form class="fullTextSearchForm"  rel="pagerForm" onsubmit="return navTabSearch(this);" action="${webUrl}/notices/0/search" method="POST">
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" href="${webUrl}/notices/new" target="dialog" width="800" height="480" rel="notices_new"><span>增加用户</span></a></li>
	...
	<table class="table"  width="1200" layoutH="138">
		...
		<tbody>
			...
			<tr target="data_id" rel="${data.id}">
				...
				<td>${data.author}</td>				
				<td width="">
				<c:forTokens var="file" items="${data.files}" delims=",">
				<a  href="${webUrl}/files?fileCode=${fn:split(file,':')[0]}" target="download"  title="是要下载这个附件么?"><span>${fn:split(file,':')[1]}</span></a> | 
				</c:forTokens>				
				</td>
				<td>
					<a title="删除" target="ajaxTodo" href="${webUrl}/notices/${data.id}?_method=DELETE" class="btnDel">删除</a>
					<a title="编辑"  target="dialog" width="800" height="480" href="${webUrl}/notices/${data.id}/edit" rel="notices_edit" class="btnEdit">编辑</a>
					<a title="查看"  target="dialog" width="800" height="480" href="${webUrl}/notices/${data.id}" rel="notices_show" class="btnLook">查看</a>
				</td>
			</tr>
			</c:forEach>
		</tbody>
