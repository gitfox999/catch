...
function initUI(_box){
	...
	//修改1=================修改上传组建参数===============
	if ($.fn.uploadify) {
		$(":file[script]", $p).each(function(){
			var $this = $(this);
			var myDate = new Date();
			var myMonth=myDate.getMonth()>8?myDate.getMonth()+1:"0"+(myDate.getMonth()+1);
			var options = {
				uploader : $this.attr("uploader") || $weburl+"/javascripts/uploadify/scripts/uploadify.allglyphs.swf",
				script : $this.attr("script"),
				cancelImg : $this.attr("cancelImg") || $weburl+"/javascripts/uploadify/cancel.png",
				queueID : $this.attr("fileQueue") || "fileQueue",
				fileDesc: $this.attr("fileDesc") || "*.jpg;*.jpeg;*.gif;*.png;*.pdf;*.xls;*.xlsx;*.doc;*.docx;*.ppt;*.pptx;*.txt;*.wps;*.rar;*.zip;*.gz",
				fileExt : $this.attr("fileExt") ||  "*.jpg;*.jpeg;*.gif;*.png;*.pdf;*.xls;*.xlsx;*.doc;*.docx;*.ppt;*.pptx;*.txt;*.wps;*.rar;*.zip;*.gz",
				folder	: $this.attr("folder") ? "/attachments/"+$this.attr("folder")+"/"+myDate.getFullYear().toString()+myMonth.toString() : "/attachments/"+myDate.getFullYear().toString()+myMonth.toString(),
				fileDataName : 'fileData',
				auto : true,
				multi : true,
				removeCompleted:false,
				buttonImg : $weburl+"/javascripts/uploadify/uploadify.gif",
				queueSizeLimit : $this.attr("queueSizeLimit") || 20,
				simUploadLimit : $this.attr("simUploadLimit") || 5,
				sizeLimit : $this.attr("sizeLimit") || 10240000,
				onError : $this.attr("onError") || uploadifyError,
	...
	//修该2======增加扩展=================
	if ($.fn.dwzExport) $("a[target=download]", $p).download();
	//修该2======增加扩展=================
