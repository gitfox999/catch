...
var DWZ = {
	...
	//修改1=====================ajax请求错跳转到登陆界面  弹出超时窗口===================================================================
	ajaxError:function(xhr, ajaxOptions, thrownError){
		var page_code=$.trim(xhr.responseText.substring(0,11));
		switch (page_code) {
		case "<!DOCTYPE":
			...
			$.ajax({
				...
				success: function(response){
					...
					if (json.statusCode==DWZ.statusCode.error){
						...
						//修改2=====================系统session超时ajax请求到登陆页面   如果是登陆页面弹出超时窗口===================================================================
						var page_code=$.trim(response.substring(0,11));
						switch (page_code){
						case "<!DOCTYPE":
							alertMsg.error(json.message || DWZ.msg("sessionTimout"), {okCall:function(){
								if ($.pdialog) $.pdialog.checkTimeout();
